...
use crate::buildin::BuildInVoting;
use crate::aggregations::parse::AggregationParserError;
use crate::parser::input::ParserInput;
use crate::parser::logic::VotingParseError::{NoRegistryProvided, NoVotingInRegistryFound, UnableToParseInt};
use crate::parser::voting_function::*;
use crate::parser::voting_function::VotingExecution::Limited;
use crate::VotingWithLimit;
...
mod test {
    ...
    fn versuch(){
        ...
        conbtext.set_value("a".to_string(), Value::from_as_int(3)).unwrap();
        conbtext.set_value("b".to_string(), Value::from_as_int(2)).unwrap();
        conbtext.set_value("c".to_string(), Value::from_as_int(1)).unwrap();
        conbtext.set_value("d".to_string(), Value::from_as_int(4)).unwrap();
        ...
        for (i, z) in x.iter_mut().enumerate() {
            z.set_value("pp".to_string(), Value::from_as_int(i)).unwrap()
        }
    ...
    fn versuch2(){
        ...
        conbtext.set_value("c".to_string(), Value::from_as_int(1)).unwrap();
        conbtext.set_value("d".to_string(), Value::from_as_int(4)).unwrap();
        conbtext.set_value(NUMBER_OF_VOTERS.to_string(), Value::from_as_int(4)).unwrap();
        ...
        for (i, z) in x.iter_mut().enumerate() {
            z.set_value("pp".to_string(), Value::from_as_int(i)).unwrap();
            z.set_value(SCORE.to_string(), Value::from_as_float(0.3 * i as f64)).unwrap();
        }
