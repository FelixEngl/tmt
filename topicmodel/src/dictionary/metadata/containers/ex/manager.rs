macro_rules! create_struct {
    ($($name: ident: $ty:ident => $method: ident: $r_typ: ty),* $(,)?) => {
        ...
        impl Default for MetadataManagerEx {
            fn default() -> Self {
                Self {
                    meta_a: Vec::new(),
                    meta_b: Vec::new(),
                    dictionary_interner: $crate::interners::DictionaryOriginStringInterner::new(),
                    $($name: $ty::new(),
                    )*
                }
            }
        ...
        impl $crate::dictionary::metadata::MetadataManager for MetadataManagerEx {
            type FieldName = $crate::dictionary::metadata::containers::ex::MetaField;
            type FieldValue = $crate::dictionary::metadata::containers::ex::ResolvableValue;
            type BoundFieldValue = $crate::dictionary::metadata::containers::ex::GenericMetadataValue;
            type UpdateError = $crate::dictionary::metadata::containers::ex::WrongResolvedValueError<$crate::dictionary::metadata::containers::ex::ResolvedValue>;
            type Metadata = $crate::dictionary::metadata::containers::ex::MetadataEx;
            type ResolvedMetadata = $crate::dictionary::metadata::containers::ex::LoadedMetadataEx;
            type Reference<'a> = $crate::dictionary::metadata::containers::ex::MetadataRefEx<'a> where Self: 'a;
            type MutReference<'a> = $crate::dictionary::metadata::containers::ex::MetadataMutRefEx<'a> where Self: 'a;
            ...
            fn unprocessed_field() -> Option<Self::FieldName> {
                Some($crate::dictionary::metadata::containers::ex::MetaField::UnalteredVocabulary)
            }
            ...
            fn get_meta_mut_for<'a>(
                ...
                let vocabulary = unsafe {
                    std::mem::transmute::<_, &'static mut dyn $crate::vocabulary::AnonymousVocabularyMut>(vocabulary)
                } as *mut dyn $crate::vocabulary::AnonymousVocabularyMut;
                Some($crate::dictionary::metadata::containers::ex::MetadataMutRefEx::new(
                    vocabulary,
                    ptr,
                    result
                ))
            ...
            fn get_or_create_meta_for<'a>(
                ...
                if word_id >= targ.len() {
                    targ.resize_with(
                        word_id + 1,
                        $crate::dictionary::metadata::containers::ex::MetadataEx::default
                    );
                }
            ...
            //         self.changed = true;
            //         self.meta_a.resize(meta_a, $crate::dictionary::metadata::containers::ex::MetadataEx::default());
            //     }
            //
            //     if meta_b > self.meta_a.len() {
            //         self.changed = true;
            //         self.meta_b.resize(meta_b, $crate::dictionary::metadata::containers::ex::MetadataEx::default());
            //     }
            // }
            ...
            fn update_ids(
                &mut self,
                update: &$crate::dictionary::metadata::update::WordIdUpdate
            ){
...
macro_rules! create_managed_implementation {
    ($($name: ident $(: $ty:ident)? => $method: ident: $r_typ: ty),+ $(,)?) => {
        $crate::dictionary::metadata::ex::manager::create_struct!(
            $($($name: $ty => $method: $r_typ,)?)+
        );
        $crate::dictionary::metadata::ex::manager::create_manager_interns!(
            $($name => $method: $r_typ,)+
        );
    }
}
...
macro_rules! update_routine_declaration_implementation {
    (interned: $field: ident, $t:ident; $($tt:tt)*) => {
        ...
        $crate::dictionary::metadata::ex::manager::update_routine_declaration_implementation!($($tt)*);
    };
...
macro_rules! update_routine_set_implementation {
    (interned: $TSelf:ident, $intern_name: ident, $t:ident; $($tt:tt)*) => {
        ...
        $crate::dictionary::metadata::ex::manager::update_routine_set_implementation!($($tt)*);
    };
...
macro_rules! implement_filter_set {
    (interned: $TSelf:ident, $target:ident, $field_name: ident, $intern_name: ident; $($tt:tt)*) => {
        paste::paste! {
            if let Some(to_iter) = $target.[<$field_name _mut>]() {
                let mut result = $crate::dictionary::metadata::ex::metadata::MetadataContainerValueGeneric::new();
        ...
        $crate::dictionary::metadata::ex::manager::implement_filter_set!(
            $($tt)*
        );
    };
    ...
    ($marker:tt: $TSelf:ident, $target:ident, $field_name: ident; $($tt:tt)*) => {
        $crate::dictionary::metadata::ex::manager::implement_filter_set!($($tt)*);
    };
    ($(;)?) => {
...
macro_rules! clean_routine_declaration_implementation {
    (interned: $field: ident, $t:ident; $($tt:tt)*) => {
        ...
        $crate::dictionary::metadata::ex::manager::clean_routine_declaration_implementation!($($tt)*);
    };
    (interned: $field: ident; $($tt:tt)*) => {
        $crate::dictionary::metadata::ex::manager::clean_routine_declaration_implementation!($($tt)*);
    };
...
macro_rules! clean_routine_counts_implementation {
    (interned: $field: ident; $($tt:tt)*) => {
        $field += 1;
        $crate::dictionary::metadata::ex::manager::clean_routine_counts_implementation!($($tt)*);
    };
    ($marker:tt: ; $($tt:tt)*) => {
        $crate::dictionary::metadata::ex::manager::clean_routine_counts_implementation!($($tt)*);
    };
        ...
        paste::paste! {
            {
                ...
                if !$var.has_content_for_field($crate::dictionary::metadata::ex::MetaField::[<$name:camel>]) {
                    $field_name -= 1;
                }
            }
        }
        $crate::dictionary::metadata::ex::manager::clean_routine_check_empty_implementation!($($tt)*);
    };
...
macro_rules! clean_routine_clear_empty_implementation {
    (interned: $TSelf:ident, $field_name: ident, $t:ident; $($tt:tt)*) => {
        ...
        $crate::dictionary::metadata::ex::manager::clean_routine_clear_empty_implementation!($($tt)*);
    };
...
macro_rules! general_conversion_right_impl {
    (interned: $TSelf: ident, $value: ident, $field_enum: ident, $target_intern: ident) => {
        {
            use $crate::dictionary::metadata::containers::ex::{GenericMetadataValue, MetaField, WrongResolvedValueError, ResolvableValue};
            Ok(
    ...
    (set: $TSelf: ident, $value: ident, $field_enum: ident) => {
        {
            use $crate::dictionary::metadata::containers::ex::{GenericMetadataValue, MetaField, WrongResolvedValueError};
            Ok(
                GenericMetadataValue::$field_enum(
                    $value
                        .try_into()
    ...
    ($($marker:tt: $target_field: ident $(, $target_intern: ident $(, $ty: ident)?)?;)*) => {
        impl MetadataManagerEx {
            fn optimize_impl(&mut self) {
                $crate::dictionary::metadata::ex::manager::update_routine_declaration_implementation!(
                ...
                $crate::dictionary::metadata::ex::manager::update_routine_set_implementation!(
                    $($marker: self $(, $target_intern $(, $ty)?)?;)*
                );
            }

            fn drop_if_unused(&mut self) {
                $crate::dictionary::metadata::ex::manager::clean_routine_declaration_implementation!(
                    $($marker: $($target_intern $(, $ty)?)?;)*
                );

                $crate::dictionary::metadata::ex::manager::clean_routine_counts_implementation!(
                    $($marker: $($target_intern)?;)*
                );



                $crate::dictionary::metadata::ex::manager::clean_routine_check_empty_implementation!(
                    $($marker: self, $target_field $(, $target_intern)?;)*
                );

                $crate::dictionary::metadata::ex::manager::clean_routine_clear_empty_implementation!(
                    $($marker: self $(, $target_intern $(, $ty)?)?;)*
                );
            }

            fn convert_value<T: Into<<Self as $crate::dictionary::metadata::MetadataManager>::FieldValue>>(
                &mut self,
                field: <Self as $crate::dictionary::metadata::MetadataManager>::FieldName,
                value: T
            ) -> Result<<Self as $crate::dictionary::metadata::MetadataManager>::BoundFieldValue, (<Self as $crate::dictionary::metadata::MetadataManager>::FieldName, $crate::dictionary::metadata::ex::ResolvableValue)> {
                use $crate::dictionary::metadata::ex::MetaField;
                let value: <Self as $crate::dictionary::metadata::MetadataManager>::FieldValue = value.into();
                paste::paste! {
                    match field {
                        $(
                            MetaField::[<$target_field:camel>] => {
                                $crate::dictionary::metadata::ex::manager::general_conversion_right_impl!(
...
use crate::dictionary::direction::{A, B};
