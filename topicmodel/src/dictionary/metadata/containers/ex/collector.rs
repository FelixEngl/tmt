...
macro_rules! create_collector_implementation {
    ...
    ($($tt:tt: $name: ident: $collect_type: ty),+ $(,)?) => {
        ...
        impl<T> MetadataCollection<T> {
            $(
                crate::dictionary::metadata::ex::collector::generate_getter!(
        ...
        impl<T: AsRef<str>> MetadataCollection<T> {
            pub fn write_into(self, target: &mut $crate::dictionary::metadata::ex::MetadataMutRefEx) {
                if let Some(dictionary_name) = self.dictionary_name {
                    target.add_dictionary(dictionary_name);
                    $(
                    if let Some(content) = self.$name {
                        crate::dictionary::metadata::ex::collector::generate_insert!(
        ...
        impl<T: AsRef<str> + Clone> MetadataCollection<T> {
            pub fn write_to(&self, target: &mut $crate::dictionary::metadata::ex::MetadataMutRefEx) {
        ...
        impl<T> MetadataCollectionBuilder<T> {
            ...
            $(crate::dictionary::metadata::ex::collector::generate_insert_builder!($tt: $name: $collect_type | T);
            )+
        }
    };
}
