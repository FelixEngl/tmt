macro_rules! convert_into {
    (voc: $reference:ident => $name: ident) => {
        paste::paste! {
            let $name = {
                use $crate::dictionary::metadata::ex::Storage;
...
macro_rules! create_cached_getter {
    (interned: $ident:ident, $interner_name: ident: $ty:ty, $($tt:tt)*) => {
        ...
        $crate::dictionary::metadata::ex::reference::create_cached_getter!($($tt)*);
    };
...
macro_rules! create_ref_implementation {
    ($($tt:tt: $name: ident $(, $interner_name: ident)?: $ty: ty),* $(,)?) => {
        ...
        pub struct MetadataRefEx<'a> {
            pub(in super) raw: &'a $crate::dictionary::metadata::containers::ex::MetadataEx,
            pub(in super) manager_ref: &'a $crate::dictionary::metadata::containers::ex::MetadataManagerEx,
            pub(in super) vocabulary: &'a dyn $crate::vocabulary::AnonymousVocabulary,
            $(pub(in super) $name: std::sync::Arc<std::sync::OnceLock<$crate::dictionary::metadata::containers::ex::Storage<'a, $ty>>>,
            )*
        }
        ...
        impl<'a> MetadataRefEx<'a> {
            pub fn new(
                raw: &'a $crate::dictionary::metadata::containers::ex::MetadataEx,
                manager_ref: &'a $crate::dictionary::metadata::containers::ex::MetadataManagerEx,
                vocabulary: &'a dyn $crate::vocabulary::AnonymousVocabulary
            ) -> Self {
                Self {
                    raw,
                    manager_ref,
        ...
        impl<'a> MetadataRefEx<'a> {
            $crate::dictionary::metadata::ex::reference::create_cached_getter!($($tt: $name $(, $interner_name)?: $ty,)*);
        }
        ...
        impl<'a> MetadataRefEx<'a> {
            ...
            pub fn create_solved(&self) -> LoadedMetadataEx {
                $(
                    $crate::dictionary::metadata::ex::reference::convert_into!($tt: self => $name);
                )+

                $crate::dictionary::metadata::ex::LoadedMetadataEx {
                    $(
...
/// A storage for data.
#[derive(Clone)]
pub(in crate::dictionary::metadata::containers) struct Storage<'a, T> {
...
impl<'a> MetadataReference<'a, MetadataManagerEx> for MetadataRefEx<'a> {
    #[inline(always)]
    fn raw(&self) -> &'a <MetadataManagerEx as crate::dictionary::metadata::MetadataManager>::Metadata {
        self.raw
    }
    ...
    #[inline(always)]
    fn into_owned(self) -> <MetadataManagerEx as crate::dictionary::metadata::MetadataManager>::Metadata {
        self.raw.clone()
    }

    #[inline(always)]
    fn into_resolved(self) -> <MetadataManagerEx as crate::dictionary::metadata::MetadataManager>::ResolvedMetadata {
        self.into()
    }
