...
// use std::sync::{Arc};
// use owning_ref::{Erased, IntoErased, OwningRef};
...
//
//
// unsafe impl<'a, T: 'a> IntoErased<'a> for HashRef<T> {
//     type Erased = HashRef<dyn Erased + 'a>;
//
...
// }
//
// pub type ErasedHashRefSlice<'u, U> = OwningRef<'u, HashRef<dyn Erased>, U>;
//
// pub type HashRefSlice<'a, T, S> = OwningRef<'a, HashRef<T>, S>;
//
...
// {
//
//     pub fn slice_owned<SliceIdx>(&self, idx: SliceIdx) -> HashRefSlice<'static, T, Slice>
//     where
...
// }
//
// impl<T> HashRef<T> where T: AsRef<str> {
//     pub fn as_str(&self) -> HashRefSlice<'static, T, str> {
//         let or = OwningRef::new(self.clone());
//         let or = or.map(|reference| { reference.as_ref() });
//         or
//     }
// }
//
