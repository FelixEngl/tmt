...
use crate::topicmodel::dictionary::{BasicDictionary, BasicDictionaryWithVocabulary, DictionaryWithMeta};
    ...
    V: BasicVocabulary<T> + AnonymousVocabulary,
{
    ...
    pub fn generate_html(&self, dir: impl AsRef<Utf8Path>) -> Result<(), std::io::Error> {
        ...
        let main = d.join("index.html");
        let a = self.generate_directed::<A>(d.join("A"))?;
        let b = self.generate_directed::<B>(d.join("B"))?;
        let html = HtmlPage::new()
